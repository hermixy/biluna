/*****************************************************************
 * $Id: rb_xmlreader.cpp 1983 2013-09-02 18:10:02Z rutger $
 ...
 * Constructor
 */
RB_XmlReader::RB_XmlReader(RB_ObjectBase* obj) {
    mRoot = obj;
    mObjectFactory = NULL;
    mPerspective = "";
    mVersion = "";
 ...
 */
bool RB_XmlReader::read(QIODevice *device) {
    if (!mRoot) {
        return false;
    }
 ...
 */
QString RB_XmlReader::errorString() const {
    return QObject::tr("%1\nLine %2, column %3")
 ...
 */
void RB_XmlReader::setNewId(bool isNewId = true) {
    mIsNewId = isNewId;
}
 ...
 * Read list container
 */
void RB_XmlReader::readListContainer() {
    if (!mReader.isStartElement()) return;
