/*****************************************************************
 * $Id: rb_signalspydialog.cpp 1965 2013-08-08 16:33:15Z rutger $
 *
 ...
 * @param target object or widget to be watched for signals and slots
 */
RB_SignalSpyDialog::RB_SignalSpyDialog(QWidget *parent, QObject* target) 
				: QDialog(parent) {
	
	textEdit = new QTextEdit(this);
	QPushButton* pushButtonClear = new QPushButton(tr("&Clear"));
	QPushButton* pushButtonClose = new QPushButton(tr("&Close"));
	
	connect(pushButtonClear, SIGNAL(clicked()), this, SLOT(clearText()));
	connect(pushButtonClose, SIGNAL(clicked()), this, SLOT(reject()));
	
	QHBoxLayout *hLayout = new QHBoxLayout;
	hLayout->addStretch(1);
	hLayout->addWidget(pushButtonClear, 0, 0);
	hLayout->addWidget(pushButtonClose, 0, 0);
    ...
    setWindowTitle(tr("RB_SignalSpyDialog"));
 ...
 * Desctructor
 */
RB_SignalSpyDialog::~RB_SignalSpyDialog() {

}

/**
 * Append text to dialog textEdit widget
 */
void RB_SignalSpyDialog::append(const QString& str) {
	textEdit->append(str);
}

/**
 * Clear text in dialog textEdit widget
 */
void RB_SignalSpyDialog::clearText() {
	textEdit->clear();
}
