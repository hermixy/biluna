/*****************************************************************
 * $Id: db_actioneditfind.cpp 2210 2015-01-22 14:59:25Z rutger $
...
DB_ActionEditFind::DB_ActionEditFind()
        : RB_Action() {
    // nothing
}


RB_GuiAction* DB_ActionEditFind::createGuiAction() {
    static RB_GuiAction* ga = NULL;

    if (!ga) {
        ga = new RB_GuiAction(DB_ActionEditFind::getName(),
                              QIcon(":/images/icons/find.png"),
                              tr("&Find"));
//        ga->setCommand("da, database");
//        ga->setKeycode("daa");
        ga->setShortcut("Ctrl+F");
        ga->setStatusTip(tr("Find text"));
        ga->setToolTip("Find text");
        ...
        ga->setFactory(DB_ActionEditFind::factory);
    }
    return ga;
}

RB_Action* DB_ActionEditFind::factory() {
    // Prerequisites
    ...
    frDlg->init();
    frDlg->hideReplaceWidgets(); // extra compared with 'find and replace'

    QWidget* focusWidget = wdgt->focusWidget();

    if (QTextEdit* te = dynamic_cast<QTextEdit*>(focusWidget)) {
        frDlg->setTextEdit(te);
    } else if (QPlainTextEdit* pte = dynamic_cast<QPlainTextEdit*>(focusWidget)) {
        frDlg->setPlainTextEdit(pte);
    } else if (QWebEngineView* wv = dynamic_cast<QWebEngineView*>(focusWidget)) {
        frDlg->setWebView(wv);
    } else {
        DB_DIALOGFACTORY->requestWarningDialog("Find not applicable,\n"
 ...
 */
//void DB_ActionEditFind::trigger() {
//    //
//    // Prepare models
//    //
