/*****************************************************************
 * $Id: rb_textpreview.cpp 2057 2014-01-06 22:05:56Z rutger $
 ...
 * @param p painter
 */
void RB_TextPreview::printText(QPrinter *printer) {
    if (!printer) {
        return;
    }


    RB_PrintTextHeaderFooter* thf = new RB_PrintTextHeaderFooter();
    thf->setDocHeader(mPageHeader, mPageHeaderHeight);
    thf->setDocContent(mDocContent);
    thf->setDocFooter(mPageFooter, mPageFooterHeight);
    thf->setPaperMargin(mMarginLeft, mMarginTop, mMarginLeft, mMarginBottom);
    thf->print(printer);
    delete thf;
    return;
...
//            / mPrinter->pageRect(QPrinter::Point).height();
    ...
    printer->scale(sourceDpiX, sourceDpiY);
// debugRectangle(p, printRect, QColor("#DDEEFF"));
    ...
    while (currentRect.intersects(contentRect)) {
        // Paint page header
        printer->save();
        printer->translate(currentX, currentY);
        printer->setFont(mPageHeader->defaultFont());
        // No print rect, it assumes that headers/footers fit within area
        mPageHeader->drawContents(printer);
        printer->restore();
        ...
        printer->save();
        printer->translate(currentX, currentY);
        printer->setFont(mDocContent->defaultFont());
// debugRectangle(p, currentRect, QColor("#DDEEFF"));
        mDocContent->drawContents(printer, currentRect);
        printer->restore();
        ...
        // Paint page footer, page footer contains to original text
        printer->save();
        printer->translate(currentX, currentY);
        printer->setFont(mPageFooter->defaultFont());
        mPageFooterNumber->drawContents(printer);
        printer->restore();
