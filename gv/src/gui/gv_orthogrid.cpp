...
#include "rb_line.h"
#include "gv_orthogrid.h"
#include "RUcs.h"



GV_OrthoGrid::GV_OrthoGrid()
  : RB_Grid(),
 ...
 * /
RVector GV_OrthoGrid::snapToGrid(GV_GraphicsView& view, const RVector& positionUcs) {
 ...
 * /
void GV_OrthoGrid::update(GV_GraphicsView& view) {
	...
	if (doc == NULL) {
                RDebug::warning(QString("GV_OrthoGrid::update: document is NULL"));
		return;
	}

    RGraphicsScene* scene = view.getScene();
    if (scene==NULL) {
        RDebug::warning("GV_OrthoGrid::update: scene is NULL");
        return;
    }
    ...
    if (projection == RS::ThreeD) {
                gridBox = RB_Box(RVector(-1000, -1000), RVector(1000, 1000));
    ...
    gridBox = RB_Box(minGridPoint, maxGridPoint);
    ...
    metaGridBox = RB_Box(minGridPoint, maxGridPoint);
}

double GV_OrthoGrid::inchAutoscale(double x, double idealSpacing) {
...
void GV_OrthoGrid::paint(GV_GraphicsView& view) {
	if (!spacing.isValid()) {
		return;
	}
	...
	for (gridPointUcs.x = min.x; gridPointUcs.x < max.x; gridPointUcs.x
			+= spacing.x) {
...
void GV_OrthoGrid::paintMetaGrid(GV_GraphicsView& view) {
    ...
    for (double x=min.x; x<max.x; x+=metaSpacing.x) {
        view.paintMetaGridLine(RB_Line(RVector(x, min.y), RVector(x, max.y)));
    }
    for (double y=min.y; y<max.y; y+=metaSpacing.y) {
        view.paintMetaGridLine(RB_Line(RVector(min.x, y), RVector(max.x, y)));
    }
}

void GV_OrthoGrid::paintRuler(GV_GraphicsView& view, RB_Ruler& ruler) {
	if (!metaSpacing.isValid()) {
		return;
	}
...
QString GV_OrthoGrid::getInfoText() {
