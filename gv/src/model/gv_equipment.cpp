/*****************************************************************
 * $Id: gv_equipment.cpp 1380 2011-03-07 20:47:51Z rutger $
 * Created: Apr 24, 2008 10:25:01 AM - rutger
 *
...
GV_Equipment::GV_Equipment(const RB_String& Id, RB_ObjectBase* p,
...
GV_Equipment::GV_Equipment(GV_Equipment* equipment) 
...
GV_Equipment::~GV_Equipment() {
	RB_DEBUG->print("GV_Equipment::~GV_Equipment()");
	// clean up children done in RB_ObjectBase and RB_ObjectContainer
	RB_DEBUG->print("GV_Equipment::~GV_Equipment() OK");
}
 ...
 */
void GV_Equipment::createMembers() {
//	RB_String uuid = RB_Uuid::createUuid().toString();
	RB_String uuid = "";
    addMember(uuid, this, "number", "-", "", RB2::MemberChar40);			// 0
//	uuid = RB_Uuid::createUuid().toString();
     ...
     * object creation it is a member of GV_Equipment. It is for convenience 
 ...
 */
int GV_Equipment::countComponent() {
 ...
 */
GV_Component* GV_Equipment::component(const RB_String& id) {
	int listCount = countObject(); 
	
	if (listCount < 1) { 
		RB_DEBUG->print("GV_Equipment::component() no objects in list WARNING");
		return NULL;
	}
	...
	if (listCount > 0 && !comp) {
		RB_DEBUG->print("GV_Equipment::component() component ERROR");
		return NULL;
	}
 ...
 * @retval node
 */
GV_Node* GV_Equipment::node(GV_Component* comp) {
	if (!comp) return NULL;
	RB_ObjectContainer* list = getContainer("GV_NodeList");
 ...
 */
void GV_Equipment::setValue(int number, const RB_Variant& var) {
	RB_Variant mVar = RB_ObjectBase::getValue(number);
	
	if (!mVar.isValid()) {
		// error
		RB_DEBUG->print("GV_Equipment::setValue(number, var) number = %i " 
 ...
 * if previousValue was not set before with original value
 */
void GV_Equipment::setValue(const RB_String& name, const RB_Variant& var) {
	RB_Variant mVar = RB_ObjectBase::getValue(name);
	
	if (!mVar.isValid()) {
		// error
		RB_DEBUG->print("GV_Equipment::setValue(number, var) name = %s " 
