...
WebView *TabWidget::newTab(bool makeCurrent)
{
    ...
    if (!m_lineEditCompleter && count() > 0) {
        ...
        completionModel->setSourceModel(DB_InternetBrowserFactory::historyManager()->historyFilterModel());
...
void TabWidget::webViewTitleChanged(const QString &title)
{
    ...
    DB_InternetBrowserFactory::historyManager()->updateHistoryItem(webView->url(), title);
}
...
void TabWidget::aboutToShowRecentTabsMenu()
{
    ...
    for (int i = 0; i < m_recentlyClosedTabs.count(); ++i) {
        ...
        QIcon icon = DB_InternetBrowserFactory::getInstance()->icon(m_recentlyClosedTabs.at(i));
        action->setIcon(icon);
