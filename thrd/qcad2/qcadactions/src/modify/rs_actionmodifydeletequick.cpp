/****************************************************************************
** $Id: rs_actionmodifydeletequick.cpp 10309 2008-04-05 13:31:40Z andrew $
**
...
RS_GuiAction* RS_ActionModifyDeleteQuick::createGuiAction() {
    static RS_GuiAction* action = NULL;

    if (action==NULL) {
        action = new RS_GuiAction(RS_ActionModifyDeleteQuick::getName(), 
            "modifydelete", 
            tr("&Delete selected"));
...
void RS_ActionModifyDeleteQuick::trigger() {
    ...
    if (en!=NULL) {
        ...
        RS_EntityContainer* parent = en->getParentContainer();
        if (parent!=NULL) {
