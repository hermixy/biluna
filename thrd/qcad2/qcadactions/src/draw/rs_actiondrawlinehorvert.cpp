/****************************************************************************
** $Id: rs_actiondrawlinehorvert.cpp 8386 2008-01-23 23:53:22Z andrew $
**
...
RS_GuiAction* RS_ActionDrawLineHorVert::createGuiAction() {
    static RS_GuiAction* action = NULL;

    if (action==NULL) {
        action = new RS_GuiAction(RS_ActionDrawLineHorVert::getName(), tr("H&orizontal / Vertical"));
...
RS_ActionInterface* RS_ActionDrawLineHorVert::factory() {
    RS_EntityContainer* container = RS_DIALOGFACTORY->getDocument();
    RS_GraphicView* graphicView = RS_DIALOGFACTORY->getGraphicView();
    
    if (container!=NULL && graphicView!=NULL) {
        RS_ActionInterface* a = new RS_ActionDrawLineHorVert(*container, *graphicView);
...
void RS_ActionDrawLineHorVert::mouseMoveEvent(RS_MouseEvent* e) {
    ...
    if (getStatus()==SetEndpoint && mouse.valid && p1.valid) {
