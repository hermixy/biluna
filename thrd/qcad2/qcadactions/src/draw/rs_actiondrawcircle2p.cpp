/****************************************************************************
** $Id: rs_actiondrawcircle2p.cpp 8821 2008-02-02 17:31:21Z andrew $
**
...
RS_GuiAction* RS_ActionDrawCircle2P::createGuiAction() {
    static RS_GuiAction* action = NULL;

    if (action==NULL) {
        action = new RS_GuiAction(RS_ActionDrawCircle2P::getName(), 
            
                "circles2p",
            tr("2 Points"));
        action->setStatusTip(tr("Draw circles with 2 points"));
        action->setCommand("circle2p,c2");
        action->setKeycode("c2");
        action->setFactory(RS_ActionDrawCircle2P::factory);
    }                                   
...
RS_ActionInterface* RS_ActionDrawCircle2P::factory() {
    RS_EntityContainer* container = RS_DIALOGFACTORY->getDocument();
    RS_GraphicView* graphicView = RS_DIALOGFACTORY->getGraphicView();
    
    if (container!=NULL && graphicView!=NULL) {
        RS_ActionInterface* a = new RS_ActionDrawCircle2P(*container, *graphicView);
