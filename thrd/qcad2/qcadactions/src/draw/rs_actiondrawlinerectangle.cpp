/****************************************************************************
** $Id: rs_actiondrawlinerectangle.cpp 8821 2008-02-02 17:31:21Z andrew $
**
...
RS_GuiAction* RS_ActionDrawLineRectangle::createGuiAction() {

    static RS_GuiAction* action = NULL;

    if (action==NULL) {
        action = new RS_GuiAction(RS_ActionDrawLineRectangle::getName(), 
            "linesrect",
                tr("&Rectangle"));
        action->setStatusTip(tr("Draw rectangles"));
        action->setCommand("linerectangle,rectangle,rectang,rect,rec,re");
        action->setKeycode("re");
        action->setFactory(RS_ActionDrawLineRectangle::factory);
    }                                   
    return action;
}
...
RS_ActionInterface* RS_ActionDrawLineRectangle::factory() {
    RS_EntityContainer* container = RS_DIALOGFACTORY->getDocument();
    RS_GraphicView* graphicView = RS_DIALOGFACTORY->getGraphicView();
    
    if (container!=NULL && graphicView!=NULL) {
        RS_ActionInterface* a = new RS_ActionDrawLineRectangle(*container, *graphicView);
...
void RS_ActionDrawLineRectangle::mouseMoveEvent(RS_MouseEvent* e) {
    ...
    if (mouse.valid && getStatus()==SetCorner2 && corner1.valid) {
        corner2 = mouse;
