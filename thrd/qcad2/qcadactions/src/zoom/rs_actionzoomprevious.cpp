/****************************************************************************
** $Id: rs_actionzoomprevious.cpp 8821 2008-02-02 17:31:21Z andrew $
**
...
RS_GuiAction* RS_ActionZoomPrevious::createGuiAction() {
    static RS_GuiAction* action = NULL;

    if (action==NULL) {
        action = new RS_GuiAction(RS_ActionZoomPrevious::getName(), 
            "zoomprevious",
                             tr("&Previous View"));
        action->setStatusTip(tr("Shows previous view"));
        action->setCommand("zoomprevious,zoomprev,zv");
        action->setKeycode("zv");
        action->setFactory(RS_ActionZoomPrevious::factory);
    }
    return action;
}
...
RS_ActionInterface* RS_ActionZoomPrevious::factory() {
