/****************************************************************************
** $Id: rs_actionblocksadd.cpp 9172 2008-02-23 15:25:20Z andrew $
**
...
RS_GuiAction* RS_ActionBlocksAdd::createGuiAction() {
    static RS_GuiAction* action = NULL;
    
    if (action==NULL) {
        action = new RS_GuiAction(RS_ActionBlocksAdd::getName(),
            "blockadd", 
            tr("&Add Block"));
        action->setStatusTip(tr("Adds a new empty block"));
        action->setCommand("ba");
        action->setKeycode("ba");
        action->setFactory(RS_ActionBlocksAdd::factory);
    }
    return action;
}
...
RS_ActionInterface* RS_ActionBlocksAdd::factory() {
    RS_EntityContainer* container = RS_DIALOGFACTORY->getDocument();
    RS_GraphicView* graphicView = RS_DIALOGFACTORY->getGraphicView();
    
    if (container!=NULL && graphicView!=NULL) {
        RS_ActionInterface* a = new RS_ActionBlocksAdd(*container, *graphicView);
