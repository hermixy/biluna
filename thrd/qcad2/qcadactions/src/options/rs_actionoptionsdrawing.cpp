/****************************************************************************
** $Id: rs_actionoptionsdrawing.cpp 4269 2007-01-20 18:51:56Z andrew $
**
...
RS_GuiAction* RS_ActionOptionsDrawing::createGuiAction() {
    static RS_GuiAction* action = NULL;

    if (action==NULL) {
        action = new RS_GuiAction(RS_ActionOptionsDrawing::getName(),  tr("Current &Drawing Preferences..."));
        action->setStatusTip(tr("Settings for the current Drawing"));
        action->setFactory(RS_ActionOptionsDrawing::factory);
    }                                   
    return action;
}
...
RS_ActionInterface* RS_ActionOptionsDrawing::factory() {
    RS_EntityContainer* container = RS_DIALOGFACTORY->getDocument();
    RS_GraphicView* graphicView = RS_DIALOGFACTORY->getGraphicView();
    
    if (container!=NULL && graphicView!=NULL) {
        RS_ActionInterface* a = new RS_ActionOptionsDrawing(*container, *graphicView);
