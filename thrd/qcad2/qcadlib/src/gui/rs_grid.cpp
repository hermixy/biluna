 ...
 */
RS_Grid::~RS_Grid() {
    RS_DEBUG->print("RS_Grid::~RS_Grid");
    if (pt!=NULL) {
        delete[] pt;
    }
    if (metaX!=NULL) {
...
void RS_Grid::update() {
    ...
    // imperial grid:
        ...
        if (unit==RS2::Inch) {
            ...
            if (scaleGrid) {
                while (graphicView->toGuiDX(gridWidth.x)<minGridSpacing) {
                    if (RS_Math::mround(gridWidth.x)>=36) {
                        gridWidth.x*=2;
                    } else if (RS_Math::mround(gridWidth.x)>=12) {
                        gridWidth.x*=3;
                    } else if (RS_Math::mround(gridWidth.x)>=4) {
                        gridWidth.x*=3;
                    } else if (RS_Math::mround(gridWidth.x)>=1) {
                        gridWidth.x*=2;
                    } else {
                        gridWidth.x*=2;
                    }
                ...
                while (graphicView->toGuiDY(gridWidth.y)<minGridSpacing) {
                    if (RS_Math::mround(gridWidth.y)>=36) {
                        gridWidth.y*=2;
                    } else if (RS_Math::mround(gridWidth.y)>=12) {
                        gridWidth.y*=3;
                    } else if (RS_Math::mround(gridWidth.y)>=4) {
                        gridWidth.y*=3;
                    } else if (RS_Math::mround(gridWidth.y)>=1) {
                        gridWidth.y*=2;
                    } else {
                        gridWidth.y*=2;
                    }
    ...
    if (gridWidth.x>1.0e-6 && gridWidth.y>1.0e-6 && 
        ...
        const int numberX = (RS_Math::mround((right-left) / gridWidth.x) + 1);
        const int numberY = (RS_Math::mround((top-bottom) / gridWidth.y) + 1);
    ...
    if (metaGridWidth.x>1.0e-6 && metaGridWidth.y>1.0e-6 && 
        ...
        numMetaX = (RS_Math::mround((metaXMax-metaXMin) / metaGridWidth.x) + 1);
        numMetaY = (RS_Math::mround((metaYMax-metaYMin) / metaGridWidth.y) + 1);
