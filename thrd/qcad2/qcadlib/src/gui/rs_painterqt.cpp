/****************************************************************************
** $Id: rs_painterqt.cpp 8332 2008-01-20 13:04:08Z andrew $
**
** Copyright (C) 2001-2003 RibbonSoft. All rights reserved.
**
** This file is part of the qcadlib Library project.
**
** This file may be distributed and/or modified under the terms of the
** GNU General Public License version 2 as published by the Free Software
** Foundation and appearing in the file LICENSE.GPL included in the
** packaging of this file.
**
** Licensees holding valid qcadlib Professional Edition licenses may use 
** this file in accordance with the qcadlib Commercial License
** Agreement provided with the Software.
**
** This file is provided AS IS with NO WARRANTY OF ANY KIND, INCLUDING THE
** WARRANTY OF DESIGN, MERCHANTABILITY AND FITNESS FOR A PARTICULAR PURPOSE.
**
** See http://www.ribbonsoft.com for further details.
**
** Contact info@ribbonsoft.com if any conditions of this licensing are
** not clear to you.
**
**********************************************************************/


#include <stdio.h>
#include <QPainterPath>

#include "rs_painterqt.h"

#include "rs_math.h"
#include "rs_application.h"
#include "rs_color.h"
#include "rs_settings.h"

/**
 * Constructor.
 */
RS_PainterQt::RS_PainterQt(QPaintDevice* pd)
        : QPainter(pd), RS_Painter() {

    lpen = RS_Pen(RS_Color(254,254,254), 
                  RS2::Width00, 
                  RS2::SolidLine);
}


/**
 * Constructor.
 */
RS_PainterQt::RS_PainterQt()
        : QPainter(), RS_Painter() {

    lpen = RS_Pen(RS_Color(254,254,254), 
                  RS2::Width00, 
                  RS2::SolidLine);
}


/**
 * Destructor
 */
RS_PainterQt::~RS_PainterQt() {}

    
    
void RS_PainterQt::moveTo(double x, double y) {
    posX = x;
    posY = y;
}


void RS_PainterQt::lineTo(double x, double y) {
    QPainter::drawLine(QPointF(posX, posY), QPointF(x, y));
    posX = x;
    posY = y;
}

/**
 * Draws a grid point at (x1, y1).
 */
void RS_PainterQt::drawGridPoint(const RS_Vector& p) {
    QPainter::drawPoint(QPointF(offset.x + p.x, offset.y + p.y));
}



/**
 * Draws a point at (x1, y1).
 */
void RS_PainterQt::drawPoint(const RS_Vector& p) {
    double x = offset.x + p.x;
    double y = offset.y + p.y;
    QPainter::drawLine(QPointF(x-1, y), QPointF(x+1, y)); 
    QPainter::drawLine(QPointF(x, y-1), QPointF(x, y+1));
}



/**
 * Draws a line from (x1, y1) to (x2, y2).
 */
void RS_PainterQt::drawLine(const RS_Vector& p1, const RS_Vector& p2) {
    QPainter::drawLine(QPointF(offset.x + p1.x, offset.y + p1.y),
                       QPointF(offset.x + p2.x, offset.y + p2.y));
}



/**
 * Draws an arc which starts / ends exactly at the given coordinates.
 *
 * @param cx center in x
 * @param cy center in y
 * @param radius Radius
 * @param a1 Angle 1 in rad
 * @param a2 Angle 2 in rad
 * @param x1 startpoint x
 * @param y1 startpoint y
 * @param x2 endpoint x
 * @param y2 endpoint y
 * @param reversed true: clockwise, false: counterclockwise
 */
/*
void RS_PainterQt::drawArc(const RS_Vector& cp, double radius,
                           double a1, double a2,
                           const RS_Vector& p1, const RS_Vector& p2,
                           bool reversed) {
    if(radius<=0.5) {
        drawGridPoint(cp);
    } else {
        int   cix;            // Next point on circle
        int   ciy;            //
        double aStep;         // Angle Step (rad)
        double a;             // Current Angle (rad)
        double linStep;       // linear step (pixels)

        // segment length in pixels
        if (drawingMode==RS2::ModePreview) {
            linStep = 20.0;
        } else {
            linStep = 6.0;
        }

        if (fabs(linStep/radius)<=1.0) {
            aStep=asin(linStep/radius);
        } else {
            aStep=1.0;
        }

        if (aStep<0.05) {
            aStep = 0.05;
        }

        if(!reversed) {
            // Arc Counterclockwise:
            if(a1>a2-1.0e-10) {
                a2+=2*M_PI;
            }
            QPainterPath pa;
            pa.moveTo(toScreenX(p1.x), toScreenY(p1.y));
            for(a=a1+aStep; a<=a2; a+=aStep) {
                cix = toScreenX(cp.x+cos(a)*radius);
                ciy = toScreenY(cp.y-sin(a)*radius);
                pa.lineTo(cix, ciy);
            }
            pa.lineTo(toScreenX(p2.x), toScreenY(p2.y));
            drawPath(pa);
        } else {
            // Arc Clockwise:
            if(a1<a2+1.0e-10) {
                a2-=2*M_PI;
            }
            QPainterPath pa;
            pa.moveTo(toScreenX(p1.x), toScreenY(p1.y));
            for(a=a1-aStep; a>=a2; a-=aStep) {
                cix = toScreenX(cp.x+cos(a)*radius);
                ciy = toScreenY(cp.y-sin(a)*radius);
                pa.lineTo(cix, ciy);
            }
            pa.lineTo(toScreenX(p2.x), toScreenY(p2.y));
            drawPath(pa);
        }
    }
}
*/



/**
 * Draws an arc.
 *
 * @param cx center in x
 * @param cy center in y
 * @param radius Radius
 * @param a1 Angle 1 in rad
 * @param a2 Angle 2 in rad
 * @param reversed true: clockwise, false: counterclockwise
 */
void RS_PainterQt::drawArc(const RS_Vector& cp, double radius,
                           double a1, double a2,
                           bool reversed) {

    if(!pdfOutput && radius<=0.5) {
        drawGridPoint(cp);
    } else {
        QPainterPath pa;
        createArc(pa, cp, radius, a1, a2, reversed);
        drawPath(pa);
    }
}



/**
 * Draws a circle.
 * @param cx center in x
 * @param cy center in y
 * @param radius Radius
 */
void RS_PainterQt::drawCircle(const RS_Vector& cp, double radius) {

    if (drawingMode==RS2::ModeXor && radius<500) {
        // This is _very_ slow for large arcs:
        QPainter::drawEllipse(QRectF(
                              offset.x + cp.x-radius,
                              offset.y + cp.y-radius,
                              2.0*radius,
                              2.0*radius));
    } else {
        /*drawArc(cp,
                radius,
                0.0, 2*M_PI,
                cp + RS_Vector(radius, 0.0),
                cp + RS_Vector(radius, 0.0),
                false);*/
        drawArc(cp,
                radius,
                0.0, 2*M_PI,
                false);
    }
}



/**
 * Draws a rotated ellipse arc.
 */
void RS_PainterQt::drawEllipse(const RS_Vector& cp,
                               double radius1, double radius2,
                               double angle,
                               double a1, double a2,
                               bool reversed) {
    
    if(!pdfOutput && radius1<=0.5 && radius2<=0.5) {
        drawGridPoint(cp);
    } else {
        QPainterPath pa;
        createEllipseArc(pa, cp, radius1, radius2, angle, a1, a2, reversed);
        drawPath(pa);
    }

    /*
    double aStep;         // Angle Step (rad)
    double a;             // Current Angle (rad)

    aStep=0.01;

    RS_Vector vp;
    RS_Vector vc(cp.x, cp.y);
    vp.set(cp.x+cos(a1)*radius1,
           cp.y-sin(a1)*radius2);
    vp.rotate(vc, -angle);
    moveTo(offset.x + vp.x,
           offset.y + vp.y);
    if(!reversed) {
        // Arc Counterclockwise:
        if(a1>a2-RS_TOLERANCE) {
            a2+=2*M_PI;
        }
        for(a=a1+aStep; a<=a2; a+=aStep) {
            vp.set(cp.x+cos(a)*radius1,
                   cp.y-sin(a)*radius2);
            vp.rotate(vc, -angle);
            lineTo(offset.x + vp.x,
                   offset.y + vp.y);
        }
    } else {
        // Arc Clockwise:
        if(a1<a2+RS_TOLERANCE) {
            a2-=2*M_PI;
        }
        for(a=a1-aStep; a>=a2; a-=aStep) {
            vp.set(cp.x+cos(a)*radius1,
                   cp.y-sin(a)*radius2);
            vp.rotate(vc, -angle);
            lineTo(offset.x + vp.x,
                   offset.y + vp.y);
        }
    }
    vp.set(cp.x+cos(a2)*radius1,
           cp.y-sin(a2)*radius2);
    vp.rotate(vc, -angle);
    lineTo(offset.x + vp.x,
           offset.y + vp.y);
    */
}



/**
 * Draws image.
 */
void RS_PainterQt::drawImg(RS_Img& img, const RS_Vector& pos,
                           double angle, const RS_Vector& factor,
                           int sx, int sy, int sw, int sh) {
    RS_DEBUG->print("RS_PainterQt::drawImg");

    if (factor.x*img.width()>10000 || factor.y*img.height()>10000) {
        RS_DEBUG->print(RS_Debug::D_WARNING, 
            "RS_PainterQt::drawImg: image too big. aborting");

        RS_SETTINGS->beginGroup("/Appearance");
        bool autoHideImages = (bool)RS_SETTINGS->readNumEntry("/AutoHideImages", 1);
        RS_SETTINGS->endGroup();
        if (autoHideImages) {
            return;
        }
    }
    
    save();

    QMatrix wm;
    wm.translate(pos.x, pos.y);
    wm.rotate(RS_Math::rad2deg(-angle));
    wm.scale(factor.x, factor.y);
    setMatrix(wm);

    if (fabs(angle)<1.0e-4) {
        RS_DEBUG->print("RS_PainterQt::drawImg: drawImage section");
        drawImage(0+sx,-img.height()+sy, img, sx, sy, sw, sh);
        RS_DEBUG->print("RS_PainterQt::drawImg: drawImage section: OK");
    }
    else {
        RS_DEBUG->print("RS_PainterQt::drawImg: drawImage rotated");
        drawImage(0,-img.height(), img);
        RS_DEBUG->print("RS_PainterQt::drawImg: drawImage rotated: OK");
    }

    restore();

    RS_DEBUG->print("RS_PainterQt::drawImg: OK");
}



void RS_PainterQt::drawTextH(int x1, int y1,
                             int x2, int y2,
                             const QString& text) {
    drawText(x1, y1, x2, y2,
             Qt::AlignRight|Qt::AlignVCenter,
             text);
}



void RS_PainterQt::drawTextV(int x1, int y1,
                             int x2, int y2,
                             const QString& text) {
    save();
    QMatrix wm = matrix();
    wm.rotate(-90.0);
    setMatrix(wm);

    drawText(x1, y1, x2, y2,
             Qt::AlignRight|Qt::AlignVCenter,
             text);

    restore();
}

void RS_PainterQt::fillRect(int x1, int y1, int w, int h,
                            const RS_Color& col) {
    QPainter::fillRect(x1, y1, w, h, col);
}


void RS_PainterQt::fillTriangle(const RS_Vector& p1,
                                const RS_Vector& p2,
                                const RS_Vector& p3) {

    QPainterPath arr;
    arr.moveTo(offset.x + p1.x,offset.y + p1.y);
    arr.lineTo(offset.x + p2.x,offset.y + p2.y);
    arr.lineTo(offset.x + p3.x,offset.y + p3.y);
    arr.closeSubpath();
    setBrush(pen().color());
    drawPolygon(arr);
    disableBrush();
}


void RS_PainterQt::setBackground(const RS_Color& color) {
    QPainter::setBackground(color);
}


void RS_PainterQt::erase() {
    QPainter::eraseRect(0,0,getWidth(),getHeight());
}


int RS_PainterQt::getWidth() {
    return device()->width();
}


int RS_PainterQt::getHeight() {
    return device()->height();
}


void RS_PainterQt::setPreviewPen() {
    setPen(QColor(0,255,255));
}

RS_Pen RS_PainterQt::getPen() {
    return lpen;
}

void RS_PainterQt::setPen(const RS_Pen& pen) {
    /*
    if (overridePen) {
        return;
    }
    */

    lpen = pen;
    if (drawingMode==RS2::ModeBW) {
        lpen.setColor(RS_Color(0,0,0));
    }
    QPen p(lpen.getColor(), RS_Math::mround(lpen.getScreenWidth()),
           RS2::rsToQtLineType(lpen.getLineType()));
    p.setJoinStyle(Qt::RoundJoin);
    p.setCapStyle(Qt::RoundCap);
    QPainter::setPen(p);
}

void RS_PainterQt::setPen(const RS_Color& color) {
    /*
    if (overridePen) {
        return;
    }
    */

    if (drawingMode==RS2::ModeBW) {
        lpen.setColor(RS_Color(0,0,0));
        QPainter::setPen(RS_Color(0,0,0));
    } else {
        lpen.setColor(color);
        QPainter::setPen(color);
    }
}

void RS_PainterQt::setPen(int r, int g, int b) {
    /*
    if (overridePen) {
        return;
    }
    */

    if (drawingMode==RS2::ModeBW) {
        setPen(QColor(0, 0, 0));
    } else {
        setPen(QColor(r, g, b));
    }
}
    
void RS_PainterQt::setOpacity(double alpha) {
    if (alpha<0.0) alpha = 0.0;
    else if (alpha>1.0) alpha = 1.0;

    QPainter::setOpacity(alpha);
}

void RS_PainterQt::disablePen() {
    /*
    if (overridePen) {
        return;
    }
    */

    lpen = RS_Pen(RS2::FlagInvalid);
    QPainter::setPen(Qt::NoPen);
}

void RS_PainterQt::setBrush(const RS_Color& color) {
    /*
    if (overridePen) {
        return;
    }
    */
    
    if (drawingMode==RS2::ModeBW) {
        QPainter::setBrush(QColor(0, 0, 0));
    } else {
        QPainter::setBrush(color);
    }
}


void RS_PainterQt::setBrush(const RS_Pixmap& pixmap) {
    /*
    if (overridePen) {
        return;
    }
    */

    QPainter::setBrush(pixmap);
}
    

void RS_PainterQt::disableBrush() {
    /*
    if (overridePen) {
        return;
    }
    */

    QPainter::setBrush(Qt::NoBrush);
}


void RS_PainterQt::drawPolygon(const RS_PointArray& a) {
    QPainter::drawPath(a);
}

void RS_PainterQt::setXORMode() {
}

void RS_PainterQt::setNormalMode() {
}

void RS_PainterQt::setClipRect(int x, int y, int w, int h) {
    QPainter::setClipRect(x, y, w, h);
    setClipping(true);
}

void RS_PainterQt::resetClipping() {
    setClipping(false);
}


