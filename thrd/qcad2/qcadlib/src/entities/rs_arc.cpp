/****************************************************************************
** $Id: rs_arc.cpp 7925 2008-01-17 18:01:07Z andrew $
**
...
const char* RS_Arc::propertyCenterX = QT_TRANSLATE_NOOP("QObject", "Center|X");
const char* RS_Arc::propertyCenterY = QT_TRANSLATE_NOOP("QObject", "Center|Y");
const char* RS_Arc::propertyRadius = QT_TRANSLATE_NOOP("QObject", "Radius");
const char* RS_Arc::propertyStartangle = QT_TRANSLATE_NOOP("QObject", "Angles|Startangle");
const char* RS_Arc::propertyEndangle = QT_TRANSLATE_NOOP("QObject", "Angles|Endangle");
const char* RS_Arc::propertyReversed = QT_TRANSLATE_NOOP("QObject", "Reversed");


/**
 * Default constructor.
 */
RS_Arc::RS_Arc(RS_EntityContainer* parentContainer,
               const RS_ArcData& d)
...
void RS_Arc::calculateBorders(bool /*visibleOnly*/) {
...
RS_VectorList RS_Arc::getRefPoints(RS2::RefMode /*refMode*/) {
    RS_VectorList ret(startpoint, endpoint, data.center);
    return ret;
}
...
RS2::Side RS_Arc::getSideOfPoint(const RS_Vector& coord) {
    if (data.reversed) {
...
void RS_Arc::moveStartpoint(const RS_Vector& pos) {
    ...
    //if (parentContainer!=NULL && parentContainer->rtti()==RS2::EntityPolyline) {
...
RS_Polyline* RS_Arc::approximateWithLines(double segmentLength) {
    RS_Polyline* polyline = new RS_Polyline(getParentContainer());
    ...
    if(!data.reversed) {
        ...
        for(a=a1+aStep; a<=a2; a+=aStep) {
            ...
            polyline->addVertex(RS_Vector(cix, ciy));
        }
    } else {
        // Arc Clockwise:
        if(a1<a2+1.0e-10) {
...
void RS_Arc::draw(RS_Painter* painter, RS_GraphicView* view,
    ...
    /*
    RS_DEBUG->print("RS_Arc::draw: radius: %f", getRadius());
    
    RS_DEBUG->print("RS_Arc::draw: start: %f/%f", 
        getStartpoint().x, getStartpoint().y);
    RS_DEBUG->print("RS_Arc::draw: end: %f/%f", 
        getEndpoint().x, getEndpoint().y);
    
    RS_DEBUG->print("RS_Arc::draw: angle1: %f", getAngle1());
    RS_DEBUG->print("RS_Arc::draw: angle2: %f", getAngle2());
    */
            ...
            if (pat->pattern[i] > 0.0) {
                if (!view->isPrinting() && da[i]*r<2) {
                    painter->drawPoint(cp + 
                        RS_Vector(r * cos(curA), -r * sin(curA)));
                }
 ...
 */
double RS_Arc::getAngleLength(bool allowForZeroLength) const {
    double ret = 0.0;
...
RS_Variant RS_Arc::getProperty(const RS_String& name, const RS_Variant& def) {
    if (name==RS_ARC_CENTER_X) {
        return data.center.x;
    } else if (name==RS_ARC_CENTER_Y) {
        return data.center.y;
    } else if (name==RS_ARC_RADIUS) {
        return data.radius;
    } else if (name==RS_ARC_STARTANGLE) {
        return RS_Math::rad2deg(data.angle1);
    } else if (name==RS_ARC_ENDANGLE) {
        return RS_Math::rad2deg(data.angle2);
    } else if (name==RS_ARC_REVERSED) {
...
RS_StringList RS_Arc::getPropertyNames(bool includeGeometry) {
...
void RS_Arc::setProperty(const RS_String& name, const RS_Variant& value) {
    if (name==RS_ARC_CENTER_X) {
