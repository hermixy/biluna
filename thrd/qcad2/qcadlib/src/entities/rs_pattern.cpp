/****************************************************************************
** $Id: rs_pattern.cpp 8330 2008-01-20 12:38:01Z andrew $
**
...
RS_Pattern::RS_Pattern(const RS_String& fileName)
        : RS_Entity(NULL), RS_EntityContainer(NULL) {
...
RS_Entity* RS_Pattern::clone() {
    RS_Pattern* ec = 
        new RS_Pattern(fileName);
    ec->loaded = loaded;
...
bool RS_Pattern::loadPattern() {
    ...
    if (!fileName.toLower().contains(".dxf")) {
        ...
        for (RS_StringList::Iterator it = patterns.begin();
            ...
            if (RS_FileInfo(*it).baseName().toLower()==fileName.toLower()) {
                path = *it;
                RS_DEBUG->print("Pattern found: %s", (const char*)path.toLatin1());
                break;
            }
        }
    }
    ...
    if (path.isEmpty()) {
        RS_DEBUG->print("No pattern \"%s\"available.", (const char*)fileName.toLatin1());
        return false;
    }
