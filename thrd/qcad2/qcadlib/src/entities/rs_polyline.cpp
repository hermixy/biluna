/****************************************************************************
** $Id: rs_polyline.cpp 7930 2008-01-17 21:23:04Z andrew $
**
 ...
 * Constructor.
 */
RS_Polyline::RS_Polyline(RS_EntityContainer* parentContainer)
...
RS_Polyline::~RS_Polyline() {
    RS_DEBUG->print("RS_Polyline::~RS_Polyline");
    RS_DEBUG->print("RS_Polyline::~RS_Polyline: OK");
}
 ...
 */
bool RS_Polyline::optimizeContours() {
    RS_DEBUG->print("RS_Polyline::optimizeContours");
...
void RS_Polyline::removeLastVertex() {
    ...
    if (last!=NULL) {
        ...
        if (last!=NULL) {
            if (last->isAtomic()) {
                data.endpoint = dynamic_cast<RS_AtomicEntity*>(last)->getEndpoint();
            }
            else {
...
double RS_Polyline::getClosingBulge() {
    if (isClosed()) {
        ...
        if (e!=NULL && e->rtti()==RS2::EntityArc) {
            return dynamic_cast<RS_Arc*>(e)->getBulge();
        }
    }
...
void RS_Polyline::updateEndpoints() {
    ...
    if (e1!=NULL && e1->isAtomic()) {
        RS_Vector v = dynamic_cast<RS_AtomicEntity*>(e1)->getStartpoint();
        setStartpoint(v);
    }
    ...
    if (e2!=NULL && e2->isAtomic()) {
        RS_Vector v = dynamic_cast<RS_AtomicEntity*>(e2)->getEndpoint();
        setEndpoint(v);
    }
}
...
RS_VectorList RS_Polyline::getRefPoints(RS2::RefMode /*refMode*/) {
    RS_VectorList ret;
    ...
    for (RS_Entity* e=firstEntity(RS2::ResolveNone);
        ...
        if (e->isAtomic()) {
            ret.set(i, dynamic_cast<RS_AtomicEntity*>(e)->getEndpoint());
        }
    }
...
void RS_Polyline::isoProject(RS2::IsoProjectionType type, double segmentLength) {
    RS_EntityContainer::isoProject(type, segmentLength);
...
void RS_Polyline::draw(RS_Painter* painter, RS_GraphicView* view, 
    ...
    RS_DEBUG->print("RS_Polyline::draw");
    ...
    RS_DEBUG->print("RS_Polyline::draw: OK");
}
