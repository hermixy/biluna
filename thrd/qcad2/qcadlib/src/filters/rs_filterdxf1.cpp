/****************************************************************************
** $Id: rs_filterdxf1.cpp 4380 2007-02-03 17:52:55Z andrew $
**
...
RS_FilterDxf1::~RS_FilterDxf1() {
    RS_DEBUG->print("RS_FilterDxf1::~RS_FilterDxf1");
    reset();
}
...
bool RS_FilterDxf1::fileImport(RS_Graphic& g, const RS_String& file, RS2::FormatType /*type*/) {
    RS_DEBUG->print("DXF1 Filter: importing file '%s'...", (const char*)file.toLatin1());
...
bool RS_FilterDxf1::readFromBuffer() {
    ...
    if(fBuf) {
        ...
        do {
            ...
            RS_DEBUG->print( "RS_FilterDxf1::readFromBuffer: dxfLine: '%s'", 
                (const char*)dxfLine.toLatin1());
                ...
                //
                    ...
                    if (!dxfCode.isEmpty()) {
                        if( dxfCode.toInt()==10 ) {
                            dxfLine=getBufLine();
                            if (!dxfLine.isEmpty()) {
                                double x = atof((const char*)dxfLine.toLatin1());
                                dxfLine=getBufLine();
                                if (!dxfLine.isEmpty()) {
                                    double y = atof((const char*)dxfLine.toLatin1());
            ...
            //
                ...
                if(dxfLine=="HATCH") {
                  do {
                    dxfCode=getBufLine();
                    if (!dxfCode.isEmpty()) code=dxfCode.toInt();
                    if(!dxfCode.isEmpty() && code!=0) {
                      dxfLine=getBufLine();
                      if (!dxfLine.isEmpty()) {
                        switch(code) {
                ...
                */
                ...
                // -----
                    ...
                    RS_DEBUG->print("RS_FilterDxf1::readFromBuffer: TEXT");
                        ...
                        if(!dxfCode.isEmpty() && code!=0) {
                            ...
                            if (!dxfLine.isEmpty() || code==1 || code==3 || code==7) {
                                ...
                                case  1:  // Text itself
                                    ...
                                    RS_DEBUG->print("RS_FilterDxf1::readFromBuffer: vtext: '%s'", 
                                        (const char*)vtext.toLatin1());
                                    break;
                                ...
                                case  7:  
                                    ...
                                    qstrncpy(vtextStyle, (const char*)getBufLine().toLatin1(), 249);
        ...
        } while(!dxfLine.isNull() && dxfLine!="EOF");
...
void RS_FilterDxf1::reset() {
    RS_DEBUG->print("RS_FilterDxf1::reset");
    if (file.isOpen()) {
        file.reset();
    }
    ...
    fSize=0;
    if(fPointer) {
        RS_DEBUG->print("RS_FilterDxf1::reset: fclose");
        fclose(fPointer);
        fPointer=0;
    }
    RS_DEBUG->print("RS_FilterDxf1::reset: OK");
}
...
RS_String RS_FilterDxf1::getBufLine() {
    ...
    str = RS_String::fromLocal8Bit(ret).trimmed();
...
bool RS_FilterDxf1::readFileInBuffer(int _bNum) {
    fPointer = fopen((const char*)name.toLatin1(), "rb");
    if(fPointer!=NULL) {
...
void RS_FilterDxf1::strDecodeDxfString(RS_String& str) {
    if (str.isEmpty())
        return;
    str.replace(QRegExp("%%c"), QString("%1").arg(QChar(0xF8))); // Diameter
    str.replace(QRegExp("%%d"), QString("%1").arg(QChar(0xB0))); // Degree
    str.replace(QRegExp("%%p"), QString("%1").arg(QChar(0xB1))); // Plus/minus
    str.replace(QRegExp("\\\\[pP]"), QString("%1").arg(QChar('\n')));
}
