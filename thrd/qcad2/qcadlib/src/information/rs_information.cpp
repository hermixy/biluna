/****************************************************************************
** $Id: rs_information.cpp 9428 2008-03-19 08:30:36Z andrew $
**
...
bool RS_Information::isTrimmable(RS_Entity* e) {
    if (e!=NULL) {
        if (e->getParentContainer()!=NULL) {
            if (e->getParentContainer()->rtti()==RS2::EntityPolyline) {
                return true;
            }
            else if (e->getParentContainer()->rtti()==RS2::EntityContainer ||
                      e->getParentContainer()->rtti()==RS2::EntityGraphic ||
                      e->getParentContainer()->rtti()==RS2::EntityBlock) {
...
bool RS_Information::isTrimmable(RS_Entity* e1, RS_Entity* e2) {
    if (e1!=NULL && e2!=NULL) {
        if (e1->getParentContainer()!=NULL && e2->getParentContainer()!=NULL) {
            if (e1->getParentContainer()->rtti()==RS2::EntityPolyline &&
               e2->getParentContainer()->rtti()==RS2::EntityPolyline &&
               e1->getParentContainer()==e2->getParentContainer()) {

                // in the same polyline
                RS_EntityContainer* pl = e1->getParentContainer();
                ...
                if (abs(idx1-idx2)==1 || abs(idx1-idx2)==pl->count()-1) {
                    ...
                    RS_DEBUG->print("RS_Information::isTrimmable: true");
                    return true;
                }
                else {
                    // not directly following entities
                    RS_DEBUG->print("RS_Information::isTrimmable: false");
                    return false;
                }
            }
            else if ((e1->getParentContainer()->rtti()==RS2::EntityContainer ||
                      e1->getParentContainer()->rtti()==RS2::EntityGraphic ||
                      e1->getParentContainer()->rtti()==RS2::EntityBlock) &&
                     (e2->getParentContainer()->rtti()==RS2::EntityContainer ||
                      e2->getParentContainer()->rtti()==RS2::EntityGraphic ||
                      e2->getParentContainer()->rtti()==RS2::EntityBlock)) {
 ...
 * RS_VectorList is set if one intersection is a tangent point.
 */
RS_VectorList RS_Information::getIntersection(RS_Entity* e1,
        RS_Entity* e2, bool onEntities) {

    //RS_DEBUG->print("RS_Information::getIntersection");
    ...
    if ((e1->rtti()==RS2::EntityEllipse && e2->rtti()==RS2::EntityEllipse) ||
            ...
            e1->isOfType(RS2::EntityDimension) || e2->isOfType(RS2::EntityDimension)) {
    ...
    if (onEntities) {
        RS_DEBUG->print("RS_Information::getIntersection: "
            "bounding rect does not overlap");
        RS_Vector min1 = e1->getMin();
        RS_Vector max1 = e1->getMax();
        RS_Vector min2 = e2->getMin();
        RS_Vector max2 = e2->getMax();
    ...
    if (e1->rtti()==RS2::EntityEllipse || e2->rtti()==RS2::EntityEllipse) {
        ...
        if (e2->rtti()==RS2::EntityLine) {
            RS_Ellipse* ellipse = dynamic_cast<RS_Ellipse*>(e1);
            ret = getIntersectionLineEllipse(dynamic_cast<RS_Line*>(e2), ellipse);
            tol = 1.0e-1;
        } 
        ...
        if (e1->rtti()==RS2::EntityConstructionLine) {
            RS_ConstructionLine* cl = dynamic_cast<RS_ConstructionLine*>(e1);
        ...
        if (e2->rtti()==RS2::EntityConstructionLine) {
            RS_ConstructionLine* cl = dynamic_cast<RS_ConstructionLine*>(e2);
        ...
        if (e1->isOfType(RS2::EntityCircle)) {
            RS_Circle* c = dynamic_cast<RS_Circle*>(e1);
        ...
        if (e2->isOfType(RS2::EntityCircle)) {
            RS_Circle* c = dynamic_cast<RS_Circle*>(e2);
        ...
        // line / line:
        if (te1->isOfType(RS2::EntityLine) && te2->isOfType(RS2::EntityLine)) {
            RS_Line* line1 = dynamic_cast<RS_Line*>(te1);
            RS_Line* line2 = dynamic_cast<RS_Line*>(te2);

            ret = getIntersectionLineLine(line1, line2);
        }

        // line / arc:
        else if (te1->isOfType(RS2::EntityLine) && te2->isOfType(RS2::EntityArc)) {

            RS_Line* line = dynamic_cast<RS_Line*>(te1);
            RS_Arc* arc = dynamic_cast<RS_Arc*>(te2);

            ret = getIntersectionLineArc(line, arc);
        }

        // arc / line:
        else if (te1->isOfType(RS2::EntityArc) && te2->isOfType(RS2::EntityLine)) {

            RS_Arc* arc = dynamic_cast<RS_Arc*>(te1);
            RS_Line* line = dynamic_cast<RS_Line*>(te2);

            ret = getIntersectionLineArc(line, arc);
        }

        // arc / arc:
        else if (te1->isOfType(RS2::EntityArc) &&
                 te2->isOfType(RS2::EntityArc)) {

            RS_Arc* arc1 = dynamic_cast<RS_Arc*>(te1);
            RS_Arc* arc2 = dynamic_cast<RS_Arc*>(te2);
    ...
    //RS_DEBUG->print("RS_Information::getIntersection: OK");
 ...
 */
RS_VectorList RS_Information::getIntersectionLineLine(RS_Line* e1,
        RS_Line* e2) {
    ...
    RS_DEBUG->print("RS_Information::getIntersectionLineLine");
    ...
    RS_DEBUG->print("RS_Information::getIntersectionLineLine: %f/%f %f/%f - %f/%f %f/%f", 
    ...
    RS_DEBUG->print("RS_Information::getIntersectionLineLine: "
        "div: %f (tolerance: %f)", div, RS_TOLERANCE);
    ...
    if (fabs(div)>1.0e-3) {
        ...
        ret = RS_VectorList(RS_Vector(xs, ys));
    }
    ...
    RS_DEBUG->print("RS_Information::getIntersectionLineLine: OK");
 ...
 */
RS_VectorList RS_Information::getIntersectionLineArc(RS_Line* line,
        RS_Arc* arc) {
 ...
 * @return One or two intersection points between given entities.
 */
RS_VectorList RS_Information::getIntersectionArcArc(RS_Arc* e1,
        RS_Arc* e2) {
 ...
 * @return One or two intersection points between given entities.
 */
RS_VectorList RS_Information::getIntersectionLineEllipse(RS_Line* line,
        RS_Ellipse* ellipse) {
    ...
    if (d < 0) {
        ...
        /*if ( (t_a < 0 || 1 < t_a) && (t_b < 0 || 1 < t_b) ) {
            ...
            if (ret1.valid && ret2.valid) {
                ret = RS_VectorList(ret1, ret2);
            }
            else {
                if (ret1.valid) {
                    ret = RS_VectorList(ret1);
                }
                if (ret2.valid) {
                    ret = RS_VectorList(ret2);
                }
            }
        //}
    } else {
        ...
        if ( 0 <= t && t <= 1 ) {
            ...
            ret = RS_VectorList(a1.lerp(a2, t));
...
bool RS_Information::isOverlapping(const RS_Vector& r1v1, const RS_Vector& r1v2,
        const RS_Vector& r2v1, const RS_Vector& r2v2) {
        
    RS_Vector min1 = RS_Vector::minimum(r1v1, r1v2);
    RS_Vector max1 = RS_Vector::maximum(r1v1, r1v2);
    RS_Vector min2 = RS_Vector::minimum(r2v1, r2v2);
    RS_Vector max2 = RS_Vector::maximum(r2v1, r2v2);
