/****************************************************************************
** $Id: rs_system.cpp 9244 2008-02-28 18:37:58Z andrew $
**
...
void RS_System::init(const RS_String& appName, const RS_String& appVersion,
    ...
    if (appDir=="") {
        this->appDir = RS_Dir::currentPath();
    } else {
        this->appDir = appDir;
    }

    RS_DEBUG->print("RS_System::init: System %s initialized.", (const char*)appName.toLatin1());
    RS_DEBUG->print("RS_System::init: App dir: %s", (const char*)appDir.toLatin1());
    initialized = true;
...
void RS_System::initLanguageList() {
    ...
    lst += RS_SETTINGS->readEntry("/Translations", "").split(';');
 ...
 */
RS_String RS_System::getEnvironmentVariable(
    const RS_String& name, const RS_String& def) {
...
void RS_System::loadTranslation(const RS_String& lang /*, const RS_String& langCmd*/) {
    RS_DEBUG->print("RS_System::loadTranslation: %s", 
        (const char*)lang.toLatin1());

    static RS_PtrList<RS_Translator> translations;
    translations.setAutoDelete(true);

    RS_StringList modules;
    modules << "qt" << "qcad" << "qcadguiqt" 
        << "qcadactions" << "qcadlib" 
        << "qcadcam" << "qcadprof" << "qcadalgo";

    // search in various directories for translations
    RS_StringList dirs = getDirectoryList("qm");
    RS_SETTINGS->beginGroup("/Paths");
    dirs += RS_SETTINGS->readEntry("/Translations", "").split(';');
    RS_SETTINGS->endGroup();
    
    for (RS_StringList::Iterator it1 = modules.begin();
         it1!=modules.end();
         ++it1) {
        for (RS_StringList::Iterator it2 = dirs.begin();
             it2!=dirs.end();
             ++it2) {
            ...
            */
            l = lang;
            //}
            RS_String file = RS_String("%1_%2").arg(*it1).arg(l);

            RS_Translator* t = new RS_Translator();
            if (t->load(file, (*it2))) {
                RS_DEBUG->print("RS_System::loadTranslation: "
    ...
    */
    ...
    for (RS_StringList::Iterator it = dirList.begin();
        ...
        if (dir.exists() && dir.isReadable()) {
            RS_StringList files = 
                dir.entryList(RS_StringList("*." + fileExtension));
