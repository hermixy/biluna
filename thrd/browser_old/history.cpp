...
QVariant HistoryModel::data(const QModelIndex &index, int role) const
{
    ...
    case Qt::DecorationRole:
        if (index.column() == 0) {
            return DB_InternetBrowserFactory::getInstance()->icon(item.url);
        }
    }
    return QVariant();
}
...
{
    if (!m_history) {
        m_history = DB_InternetBrowserFactory::historyManager();
...
HistoryDialog::HistoryDialog(QWidget *parent, HistoryManager *setHistory) : QDialog(parent)
{
    ...
    if (!history)
        history = DB_InternetBrowserFactory::historyManager();
    setupUi(this);
