/*****************************************************************
 * $Id: crm_actionactivityreport.cpp 2156 2014-08-13 10:20:33Z rutger $
...
CRM_ActionActivityReport::CRM_ActionActivityReport()
        : RB_Action() {
    // nothing
}


RB_GuiAction* CRM_ActionActivityReport::createGuiAction() {
    static RB_GuiAction* ga = NULL;

    if (!ga) {
        ga = new RB_GuiAction(CRM_ActionActivityReport::getName(),
                              // QIcon(":/images/icons/group_edit_crm.png"),
                              tr("&Activity Report"));
        ga->setStatusTip(tr("View activity report"));
        ga->setFactory(CRM_ActionActivityReport::factory);
    }
    return ga;
}

RB_Action* CRM_ActionActivityReport::factory() {
    RB_Action* a = new CRM_ActionActivityReport();
 ...
 */
void CRM_ActionActivityReport::trigger() {
    ...
    DB_ACTIONFACTORY->enableEditMenu(CRM_DialogFactory::WidgetActivityReport);
    ...
    RB_MdiWindow* mdiWin = CRM_DIALOGFACTORY->getMdiWindow(
            CRM_DialogFactory::WidgetActivityReport);
