/*****************************************************************
 * $Id: crm_campaigntargetwidget.cpp 2210 2015-01-22 14:59:25Z rutger $
 ...
 * Constructor
 */
CRM_CampaignTargetWidget::CRM_CampaignTargetWidget(QWidget *parent)
        : RB_Widget(parent) {
 ...
 * Destructor
 */
CRM_CampaignTargetWidget::~CRM_CampaignTargetWidget() {
    if (mTargetModel) {
        delete mTargetModel;
    }
    ...
    // Do not delete mActivityModel because it might be shared with CRM_ActivityWidget
    RB_DEBUG->print("CRM_CampaignTargetWidget::~CRM_CampaignTargetWidget() OK");
}

/**
 * Initialize widget and models
 */
void CRM_CampaignTargetWidget::init() {
    //
    // 1. Set model for customer mapper
    //
    mCampaignModel = CRM_MODELFACTORY->getModel(CRM_ModelFactory::ModelCampaign); // shared
    mCampaignModel->setRoot(CRM_MODELFACTORY->getRootId());
    ...
    mTargetModel = CRM_MODELFACTORY->getModel(CRM_ModelFactory::ModelCampaignTarget); // shared
 ...
 */
bool CRM_CampaignTargetWidget::fileSave(bool withSelect) {
...
void CRM_CampaignTargetWidget::filePdf(QPrinter* pr) {
    if (isWindowModified()) {
...
void CRM_CampaignTargetWidget::filePrint(QPrinter* pr) {
    if (isWindowModified()) {
        CRM_DIALOGFACTORY->requestWarningDialog(tr("Please save your data first."));
        return;
    }
...
void CRM_CampaignTargetWidget::filePrintPreview(QPrinter* pr) {
    if (isWindowModified()) {
        CRM_DIALOGFACTORY->requestWarningDialog(tr("Please save your data first."));
        return;
    }
...
void CRM_CampaignTargetWidget::print(QPrinter* pr, bool isPreview) {
    if (!tvTarget->currentIndex().isValid()) {
        CRM_DIALOGFACTORY->requestWarningDialog(tr("No target group selected."));
        return;
    }
    ...
    CRM_SqlCommonFunctions utilSql;
    utilSql.getTemplateId(query, targetId);
    RB_ObjectBase* docObj = NULL;

    if (query.next()) {
        docObj = CRM_OBJECTFACTORY->newSingleObject("CRM_Template");
        docObj->setId(query.value(0).toString());
        docObj->dbRead(CRM_MODELFACTORY->getDatabase());
    } else {
        CRM_DIALOGFACTORY->requestWarningDialog(tr("No campaign document."));
        return;
    }

    query.clear();
    QString groupId = mTargetModel->getCurrentValue("group_idx", RB2::RoleOrigData).toString();
    utilSql.getTargetList(query, groupId);

    RB_ObjectContainer* targetList = new RB_ObjectContainer("", NULL, "CRM_TargetExtendedList");
    ...
    if (isPreview) {
        ...
        preparePrintText(utilTxt, docObj, obj);
        utilTxt->print(pr);
    } else {
        int pageNo = 1;
        ...
        mActivityModel = CRM_MODELFACTORY->getModel(CRM_ModelFactory::ModelActivity, false);
...
void CRM_CampaignTargetWidget::addLogActivity(RB_ObjectBase* doc, RB_ObjectBase* target) {
    ...
    mActivityModel->setData(idx, target->getValue("customerno").toString() + "LOG" + dt);
 ...
 * Button add clicked
 */
void CRM_CampaignTargetWidget::on_pbAddCampaign_clicked() {
 ...
 */
void CRM_CampaignTargetWidget::on_pbDeleteCampaign_clicked() {
 ...
 */
void CRM_CampaignTargetWidget::on_pbSave_clicked() {
    fileSave(true);
}

void CRM_CampaignTargetWidget::on_pbFilterOnCampaign_clicked() {
    ...
    RB_String filter = "`crm_campaign`.`campaigncode` LIKE '";
    filter += str + "'";
    mCampaignModel->setWhere(filter);
}

void CRM_CampaignTargetWidget::on_pbFilterOffCampaign_clicked() {
 ...
 */
void CRM_CampaignTargetWidget::on_ileTemplate_clicked() {
    if (!tvCampaign->currentIndex().isValid()) {
        CRM_DIALOGFACTORY->requestWarningDialog(tr("No campaign selected.\n"
 ...
 */
void CRM_CampaignTargetWidget::slotUpdateEndDateTime() {
 ...
 */
void CRM_CampaignTargetWidget::on_pbAddTarget_clicked() {
 ...
 */
void CRM_CampaignTargetWidget::on_pbDeleteTarget_clicked() {
    int ret = CRM_DIALOGFACTORY->requestYesNoDialog(
            tr("Delete contact ..."),
            tr("This action cannot be undone.\n"
 ...
 */
void CRM_CampaignTargetWidget::on_pbFilterOnTarget_clicked() {
    ...
    CRM_SqlCommonFunctions utilSql;
    RB_String filter = "`crm_campaigntarget`.";
    utilSql.substrIdxContent("group_idx", filter);
    filter += " LIKE '";
    filter += str + "'";
 ...
 */
void CRM_CampaignTargetWidget::on_pbFilterOffTarget_clicked() {
 ...
 */
void CRM_CampaignTargetWidget::on_ileContactGroup_clicked() {
    if (!tvTarget->currentIndex().isValid()) {
        CRM_DIALOGFACTORY->requestWarningDialog(tr("No target selected.\n"
                                                   "Please select a target first."));
        return;
    }

    RB_Dialog* dlg = CRM_DIALOGFACTORY->getDialog(CRM_DialogFactory::DialogSelectGroup);
 ...
 */
void CRM_CampaignTargetWidget::on_pbPreview_clicked() {
 ...
 */
void CRM_CampaignTargetWidget::on_pbPrint_clicked() {
 ...
 */
void CRM_CampaignTargetWidget::on_pbPdf_clicked() {
 ...
 */
RB_String CRM_CampaignTargetWidget::getHelpSubject() const {
    return objectName();
}

bool CRM_CampaignTargetWidget::closeWidget() {
 ...
 */
void CRM_CampaignTargetWidget::changeEvent(QEvent *e) {
 ...
 */
void CRM_CampaignTargetWidget::setTargetList(RB_ObjectContainer* targetList,
                                             QSqlQuery& q) {
    ...
    12 primaryaddress5
    13 primaryaddress6
    14 customerno (ACC_Customer)
    15 mname (ACC_Customer)
    16 id (ACC_Contact)
...
void CRM_CampaignTargetWidget::preparePrintText(RB_PrintTextHeaderFooter* util,
    ...
    util->addContentReplace(QString("{$ACC_Customer.mname}"), targetObj->getValue("mname").toString());
    util->addContentReplace(QString("{$ACC_Customer.customerno}"), targetObj->getValue("customerno").toString());

    util->addContentReplace(QString("{$CRM_Template.reference}"), docObj->getValue("reference").toString());
