/*****************************************************************
 * $Id: acc_custcostpartwidget.cpp 2210 2015-01-22 14:59:25Z rutger $
 ...
 * Constructor
 */
ACC_CustCostPartWidget::ACC_CustCostPartWidget(QWidget *parent)
        : RB_Widget(parent) {
 ...
 * Destructor
 */
ACC_CustCostPartWidget::~ACC_CustCostPartWidget() {
    mModel->revert();
    delete mModel;
    RB_DEBUG->print("ACC_CustCostPartWidget::~ACC_CustCostPartWidget() OK");
}

/**
 * Initialize widget and models
 */
void ACC_CustCostPartWidget::init() {
    //
    // 1. Set model for customer mapper
    //
    mModel = ACC_MODELFACTORY->getModel(ACC_ModelFactory::ModelCustCostPart); // shared
    // mModel->setRoot(ACC_MODELFACTORY->getRootId()); not here

    // mapper can only be used once on a field, therefore:
    mCustModel = ACC_MODELFACTORY->getModel(ACC_ModelFactory::ModelCustomer); // shared
 ...
 */
bool ACC_CustCostPartWidget::fileSave(bool withSelect) {
 ...
 * Button add clicked
 */
void ACC_CustCostPartWidget::on_pbAdd_clicked() {
    RB_MmProxy* custModel = ACC_MODELFACTORY->getModel(ACC_ModelFactory::ModelCustomer);

    if (!custModel || !custModel->getProxyIndex().isValid()) {
 ...
 */
void ACC_CustCostPartWidget::on_pbDelete_clicked() {
 ...
 */
void ACC_CustCostPartWidget::on_pbSave_clicked() {
    fileSave(true);
}

void ACC_CustCostPartWidget::on_pbFilterOn_clicked() {
...
void ACC_CustCostPartWidget::on_pbFilterOff_clicked() {
    mModel->setWhere("");
    leFilter->setText("");
}
 ...
 */
void ACC_CustCostPartWidget::on_ileCostGroup_clicked() {
 ...
 */
void ACC_CustCostPartWidget::on_ileCostCenter_clicked() {
    if (!tv->currentIndex().isValid()) {
        ACC_DIALOGFACTORY->requestWarningDialog(tr("No cost center part selected.\n"
                                                   "Please select a part first."));
        return;
 ...
 */
void ACC_CustCostPartWidget::slotParentRowChanged(const QModelIndex& current,
                                                  const QModelIndex& /* previous */) {
    if (!current.isValid()) {
        leCustomer->setText("-");
        return;
    }
    QModelIndex idx = mCustModel->index(current.row(), mCustModel->fieldIndex("mname"),
                                        current.parent());
    RB_String str = mCustModel->data(idx).toString();
    leCustomer->setText(str);
}

/**
 * @returns the help subject, in this case the widget objectname
 */
RB_String ACC_CustCostPartWidget::getHelpSubject() const {
    return objectName();
}

bool ACC_CustCostPartWidget::closeWidget() {
 ...
 */
void ACC_CustCostPartWidget::changeEvent(QEvent *e) {
